...
def from_config(dataset_config):
    ...
    edge_calc_method = dataset_config["preprocess_params"].get("edge_calc_method", "ocp")
...
class DataProcessor:
    ...
    def json_wrap(self):
        ...
        original_structures = original_data["data"]

        dict_structures = []
        # y = []
        # y_dim = (
        #     len(original_structures[0]["n"])
        #     if isinstance(original_structures[0]["n"], list)
        #     else 1
        # )
        for i, data in enumerate(original_structures):
            structure = data[0]
            d = {}
            lattice = structure["lattice"]
            d["lattice_matrix"]= lattice["matrix"]
            d["a"]= lattice["a"]
            d["b"]= lattice["b"]
            d["c"]= lattice["c"]
            d["alpha"]= lattice["alpha"]
            d["beta"]= lattice["beta"]
            d["gamma"]= lattice["gamma"]
            d["volumne"]= lattice["volume"]
            d["cell"] = torch.tensor(lattice["matrix"], device=self.device, dtype=torch.float).unsqueeze(0)
            sites= structure["sites"]
            positions=[]
            atomic_numbers=[]
            for site in sites:
                positions.append(site["xyz"])
                atomic_numbers.append(self.element_to_number(site["species"][0]["element"]))
            d["positions"] = torch.tensor(positions, device=self.device, dtype=torch.float)    
            d["atomic_numbers"] = torch.tensor(atomic_numbers, device=self.device, dtype=torch.long)
            d["structure_id"] = structure["@module"] + "_" + structure["@class"]
            ...
            if self.additional_attributes:
             for attr in self.additional_attributes:
                if attr in structure:
                    d[attr] = torch.tensor(structure[attr], device=self.device, dtype=torch.float)
            d["y"] = data[1]
            dict_structures.append(d)
