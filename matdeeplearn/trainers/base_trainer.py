...
class BaseTrainer(ABC):
    ...
    def load_pre_trained_weights(self, load_training_state=False):
        ...
        for x in range(len(self.model)):
            ...
            if load_training_state == True:
                if load_model[x].get("optimizer"): 
                    self.optimizer[x].load_state_dict(load_model[x]["optimizer"])
                if load_model[x].get("scheduler"):     
                    self.scheduler[x].scheduler.load_state_dict(load_model[x]["scheduler"])
                ...
                if load_model[x].get("seed"): 
                    seed = load_model[x]["seed"]
                    self.set_seed(seed)
                if load_model[x].get("scaler"):
                    self.scaler.load_state_dict(load_model[x]["scaler"])
