...
class TorchMD_ET_Early(BaseModel):
    ...
    def _forward(self, data):
        ...
        if self.prediction_level == "graph":
            ...
            for i in range(0, len(self.post_lin_list) - 1):
                ...
                x = self.dropout(x)
            x = self.post_lin_list[-1](x) 
        #print(f"[DEBUG] Post-dropout activations: {x}")  # Check for NaN/Inf
        return x
    ...
    def forward(self, data):
        ...
        return output        
    #METHODS FOR MONTE CARLO DROPOUT INFERENCE
    ...
    def mc_dropout_forward(self, data, n_samples=10):
        """
        Monte Carlo dropout forward pass to estimate uncertainty
        ...
        Returns:
            Dictionary containing:
                - output: Mean of MC predictions
                - uncertainty: Variance of MC predictions (uncertainty estimate)
                - all_predictions: All MC predictions
                - other standard outputs like gradients if required
        """
        # Enable dropout while keeping model in eval mode
        self.eval()
        self.enable_dropout()
        ...
        # Perform n_samples forward passes
        predictions = []
        pos_grads = []
        cell_grads = []
        
        # Temporarily set gradient calculation if needed
        needs_grad = data.pos.requires_grad
        if self.gradient and not needs_grad:
            data.pos.requires_grad = True
            if hasattr(data, 'displacement') and data.displacement is not None:
                data.displacement.requires_grad = True
        ...
        for _ in range(n_samples):
            ...
            if self.gradient:
                ...
                pos_grads.append(forces)
                cell_grads.append(stress)
        
        # Restore original requires_grad state
        if self.gradient and not needs_grad:
            data.pos = data.pos.detach()
            if hasattr(data, 'displacement') and data.displacement is not None:
                data.displacement = data.displacement.detach()
        ...
        result = {
            ...
            "uncertainty": variance,  # This is your uncertainty estimate
            "all_predictions": predictions_tensor,
        }
        
        # Add gradients if they were calculated
        if self.gradient:
            pos_grads_tensor = torch.stack(pos_grads)
            cell_grads_tensor = torch.stack(cell_grads)
            
            mean_pos_grad = torch.mean(pos_grads_tensor, dim=0)
            var_pos_grad = torch.var(pos_grads_tensor, dim=0)
            
            mean_cell_grad = torch.mean(cell_grads_tensor, dim=0)
            var_cell_grad = torch.var(cell_grads_tensor, dim=0)
            
            result["pos_grad"] = mean_pos_grad
            result["pos_grad_uncertainty"] = var_pos_grad
            result["cell_grad"] = mean_cell_grad
            result["cell_grad_uncertainty"] = var_cell_grad
        else:
            result["pos_grad"] = None
            result["cell_grad"] = None
