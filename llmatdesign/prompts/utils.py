...
def format_prompt(
    ...
    history = get_past_modifications(suggestions_list, structures_list, properties_list, reflections_list)
    if history is None:
        history = ""
    ...
    prompt= prompt.replace("<target_value>", f"{target_property:.2f}")
...
def get_reflection_str(llm_response):
    code = extract_python_code(llm_response)
    code = code.strip()
